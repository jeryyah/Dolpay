...
import { Zap, History, MessageCircle, HelpCircle, Code, LogOut, ShieldCheck, User as UserIcon, Crown, Trophy, Languages, ArrowLeft, Heart, Gift, Key } from "lucide-react";
    ...
    <header className="sticky top-0 z-40 border-b border-border/60 bg-background/85 backdrop-blur-md">
      <div className="container mx-auto px-4 h-14 flex items-center justify-between gap-2">
        ...
        <nav className="hidden md:flex items-center gap-1 text-sm">
          ...
          {user && (
            <Link href="/my-keys"><a className="px-3 py-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-all flex items-center gap-1.5"><Key className="w-4 h-4" />My Keys</a></Link>
          )}
        ...
        <div className="flex items-center gap-2">
          ...
          {user ? (
            <>
              ...
              <button
                onClick={() => {
                  if (confirm("Yakin mau logout?")) { logout(); navigate("/login"); }
                }}
                className="text-xs font-bold text-destructive border border-destructive/30 hover:bg-destructive/10 flex items-center gap-1 px-2.5 py-1.5 rounded-lg"
                title="Logout"
              >
