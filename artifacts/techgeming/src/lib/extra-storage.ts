...
export function downloadBackup() {
  ...
  a.href = url;
  a.download = `techgeming-backup-${new Date().toISOString().slice(0, 10)}.json`;
  a.click();
...
export interface ChatAttachment {
  ...
  /** Size in bytes. */
  size: number;
  /** Inline data URL (base64). Kept small (<= 4MB) so it round-trips through JSON storage. */
  dataUrl: string;
}
...
// belum sempat ter-POST tetap dipertahankan. Dedupe berdasarkan `id` —
// client mengirim id-nya saat POST sehingga server menggunakan id yang sama.
// Fallback: tetap dedup juga berdasar at|from|text untuk pesan lama.
function mergeChatMaps(local: ChatMap, remote: ChatMap): ChatMap {
  ...
  for (const [uid, localMsgs] of Object.entries(local)) {
    ...
    const seenIds = new Set(remoteMsgs.map((m) => m.id));
    const seenSig = new Set(remoteMsgs.map((m) => `${m.at}|${m.from}|${m.text}`));
    const extras = localMsgs.filter(
      (m) => !seenIds.has(m.id) && !seenSig.has(`${m.at}|${m.from}|${m.text}`),
    );
...
export function sendChat(
  ...
  // 1) Optimistic insert lokal supaya UI langsung tampak. Generate id+at di
  //    client lalu kirim juga ke server supaya server pakai id yang sama —
  //    mencegah duplikasi pesan saat hasil POST/sync ditarik kembali.
  const id = rid();
  const at = new Date().toISOString();
  const m = getChatMap();
  const cur = m[userId] || [];
  cur.push({ id, from, text: trimmed, at, read: false, attachment });
  ...
  void fetch(CHAT_API, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ id, userId, from, text: trimmed, at, attachment }),
  })
    .then(async (r) => {
